...
class ManyEnvs(gym.Env):
    ...
    def seed(self, seeds):
        for seed, env in zip(seeds, self.envs):
            env.seed=seed

    def reset(self):
        many_obs = [env.reset(seed=env.seed)[0] for env in self.envs]
        self.done = [False] * len(self.envs)
        return many_obs
...
def batch_evaluate(agent, env_name, seed, episodes, return_obss_actions=False, pixel=False):
    ...
    for i in range((episodes + num_envs - 1) // num_envs):
        # print(f" :::: started evaluating {i+1}th episode...")
        ...
        while (num_frames == 0).any() and cur_num_frames <= 250:
            ...
            many_obs, reward, done, _, _ = env.step(action)
            agent.analyze_feedback(reward, done)
            done = np.array(done)
            ...
            # print(f" :::: episode_batch: {i+1} :: frames: {cur_num_frames} :: done: {already_done.sum()}")

        print(f" :::: completed evaluating {(i+1) * num_envs} episodes...")
